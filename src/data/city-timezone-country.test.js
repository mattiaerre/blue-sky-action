...
describe.skip('city timezone country', () => {
  Object.keys(home).forEach(key => {
    test(home[key].city.name, async () => {
      ...
      url = `${process.env.TIMEZONEDB_BASE_URL}/v2/get-time-zone?key=${process.env.TIMEZONEDB_API_KEY}&format=json&by=position&lat=${city.coord.lat}&lng=${city.coord.lon}`;
      ...
      url = `${process.env.RESTCOUNTRIES_BASE_URL}/rest/v2/alpha/${city.country}`;
